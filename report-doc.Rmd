...
```{r fig.id = 'industry-input', results = 'asis', fig.cap = "Direct Capital Expenditure Summary by Industry", include = "Summary input data" %in% params$include_graphs, fig.asp = 1}
...
This project is estimated to generate `r scales::comma_format()(sum(data[["emp"]][data[["emp"]]$type %in% c("Direct Employment", "Flow on Employment"), ]$value))` jobs over `r max(as.numeric(data[["emp"]]$year)) - min(as.numeric(data[["emp"]]$year)) + 1` year(s). `r  round(sum(data[["emp"]][data[["emp"]]$type == "Direct Employment",]$value))` of these jobs are generated as a result of the direct capital expenditure, and the remaining `r  round(sum(data[["emp"]][data[["emp"]]$type == "Flow on Employment",]$value))` jobs are generated through this expenditure flowing through to other sectors of the economy.
...
```{r tab.id = 'emp-direct-table', tab.cap = "Estimated Direct employment impacts by industry by year", include = "Direct employment impacts" %in% params$include_tables, results = 'asis'}
...
```{r tab.id = "emp-flow-on-table",tab.cap = "Estimated Flow on employment impacts by industry by year", include = "Flow-on employment impacts" %in% params$include_tables, results = 'asis'}
...
```{r tab.id = "emp-total-table", tab.cap = "Estimated Total employment impacts by industry by year", include = "Total employment impacts" %in% params$include_tables, results = 'asis'}
...
```{r tab.id = 'emp-total-total-table', tab.cap = "Estimated employment impacts by year", include = "Summary employment impacts" %in% params$include_tables, results = 'asis'}
...
This project is estimated to generate `r round(sum(data[["grp"]][data[["grp"]]$type %in% c("Direct GRP", "Flow on GRP"), ]$value))` million dollars in Gross Regional Product (GRP) over `r max(as.numeric(data[["grp"]]$year)) - min(as.numeric(data[["grp"]]$year)) + 1` year(s). `r round(sum(data[["grp"]][data[["grp"]]$type == "Direct GRP",]$value))` of this is generated as a result of the direct capital expenditure, and the remaining `r  round(sum(data[["grp"]][data[["grp"]]$type == "Flow on GRP",]$value))` is generated through this expenditure flowing through to other sectors of the economy.
