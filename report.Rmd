...
params:
  ...
  author: "EIAT-APP"
  data: !r matrix(round(runif(19*10, 0, 1)), nrow = 19, ncol = 10, dimnames = list(eiat:::anzsic_swap$name, 2022:(2022+9)))
  region: "Adelaide"
...
```{r industry-input, results = 'asis', fig.cap = "Direct Capital Expenditure Summary by Industry", include = "Summary input data" %in% params$include_graphs}
...
This project is estimated to generate `r scales::comma_format()(sum(emp_data$value))` jobs over `r max(as.numeric(emp_data$year)) - min(as.numeric(emp_data$year)) + 1` year(s). `r  round(sum(emp_data[emp_data$type == "Direct Employment",]$value))` of these jobs are generated as a result of the direct capital expenditure, and the remaining `r  round(sum(emp_data[emp_data$type == "Flow on Employment",]$value))` jobs are generated through this expenditure flowing through to other sectors of the economy.
...
emp_data %>% 
  ...
  kbl(digits = 0,
      caption = "Estimated Direct employment impacts by industry by year.",
      booktabs = TRUE) %>%
  kable_styling(full_width = TRUE, latex_options = c("hold_position")) %>%
  column_spec(1, width = "7cm") %>%
  row_spec(nrow(params$data) + 1, bold = T)
...
emp_data %>% 
  ...
  adorn_totals() %>% 
  kbl(digits = 0,
      caption = "Estimated Flow on employment impacts by industry by year.",
      booktabs = TRUE)  %>%
  kable_styling(full_width = TRUE, latex_options = c("hold_position")) %>%
  column_spec(1, width = "7cm") %>%
  row_spec(nrow(params$data) + 1, bold = T)
...
emp_data %>% 
  ...
  adorn_totals() %>% 
  kbl(digits = 0,
      caption = "Estimated Total employment impacts by industry by year.",
      booktabs = TRUE)  %>%
  kable_styling(full_width = TRUE, latex_options = c("hold_position")) %>%
  column_spec(1, width = "7cm") %>%
  row_spec(nrow(params$data) + 1, bold = T)
...
emp_data %>% 
  ...
  kbl(digits = 0,
      caption = "Estimated employment impacts by year.",
      booktabs = TRUE)  %>%
  kable_styling(full_width = TRUE, latex_options = c("hold_position")) %>%
...
This project is estimated to generate `r round(sum(grp_data$value))` million dollars in Gross Regional Product (GRP) over `r max(as.numeric(grp_data$year)) - min(as.numeric(grp_data$year)) + 1` year(s). `r round(sum(grp_data[grp_data$type == "Direct GRP",]$value))` of this is generated as a result of the direct capital expenditure, and the remaining `r  round(sum(grp_data[grp_data$type == "Flow on GRP",]$value))` is generated through this expenditure flowing through to other sectors of the economy.
...
if (shiny::isRunning()) {setProgress(1)}
```
